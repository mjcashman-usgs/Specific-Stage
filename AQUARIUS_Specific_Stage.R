...
quantile.selection<-c(0.3,0.5,0.7,0.8,0.85,0.9,0.92,0.94,0.96,0.97,0.98,0.99,0.995,0.999) #Manual Range
Target_Quantile <- 3 #target Number in Sequence

#####Data Preparation####
#Import data from working directory
getwd()
setwd("D:/R_Projects/Specific Stage/") #Set WD
mainDir <- "D:/R_Projects/Specific Stage/AQUARIUS DATA"
exportDir <- "D:/R_Projects/Specific Stage/"
...
#Extract Site Numbers to Work On
  ...
  #Automatic
    library(stringr)
    setwd("D:/R_Projects/Specific Stage/AQUARIUS DATA") #Set WD
    file.list<-list.files()
    setwd("D:/R_Projects/Specific Stage/") #Reset WD
    
    Site_List <- unique(str_extract(subset(file.list, regexpr(".csv",file.list)>0),"(?<=@)\\d{8}(?=.)"))
    
 print(paste0("Runnings Gage #s in ",getwd()))
 print(Site_List)
# Site_List<-Site_List[91:94]
  Site_List<-c("05320500", "08353000","08353099","50055000","01644371","01644372","01644380","01644388",
               "01644390", "01643395","01644375") #Manual
...
for(j in 1:length(Site_List)){
  ...
  if (file.exists(subDir)){  } else {
    dir.create(file.path(exportDir, subDir))  }
  ...
  DV_MEAN <- fread(paste0("D:/R_Projects/Specific Stage/AQUARIUS DATA/",Path),skip=14)
  
  print(paste0("Importing Instantaneous Discharge Values"))
  Path<-subset(Current_Site_Files, regexpr("Discharge",Current_Site_Files)>0&regexpr("Mean",Current_Site_Files)<0)
  UV_DISCHARGE <- fread(paste0("D:/R_Projects/Specific Stage/AQUARIUS DATA/",Path),skip=14)
  
  print(paste0("Importing Instantaneous Stage Values"))
  Path<-subset(Current_Site_Files, regexpr("Gage_height",Current_Site_Files)>0)
  UV_STAGE <- fread(paste0("D:/R_Projects/Specific Stage/AQUARIUS DATA/",Path),skip=14)
  
   
  #Combine
  print(paste0("Merging Instantaneous Values"))
  timestamp<-colnames(UV_DISCHARGE)[2]
  Combined_Q_S <- merge(UV_DISCHARGE,UV_STAGE, by=timestamp, all=TRUE) 
  ...
  DV_MEAN$date.time = substr(DV_MEAN$date.time,1,nchar(DV_MEAN$date.time)-4)
  
  DV_MEAN$date<-as.Date(DV_MEAN$date.time)
  ...
  DV_MEAN$date.time1<-as.POSIXct(DV_MEAN$date.time, tz=tz, format="%Y-%m-%d %H:%M:%S")
  ...
  Combined_Q_S$date<-as.Date(Combined_Q_S$date.time)
  timestamp
  Combined_Q_S$date.time1<-as.POSIXct(Combined_Q_S$date.time, tz=tz, format="%Y-%m-%d %H:%M:%S")
  ...
  All.Runs <- melt((Runs.nos), id.vars = c("date.time1"),measure.vars = c("Q","S"))
  ...
  Plot1a<-(ggplot(data=subset(Combined_Q_S), aes(x=date.time1, y=Q))+
  ...
  Plot1aa<-(ggplot(data=subset(Combined_Q_S), aes(x=date.time1, y=Q))+
              ggtitle(paste0("UV Hydrograph at USGS Gage #", Current_Site))+
              theme(plot.title=element_text(hjust=0.5))+
              geom_line(alpha=0.5)+
              ylab("UV_Instant Discharge (cfs)")+
  ...
  Plot0<-ggplot(data=subset(All.Runs,variable=="Q"), aes(x=date.time1,y=value,color=as.factor(L2)))+
    ...
    scale_x_datetime(date_labels = "%b\n%Y",limits = UVStage_Range.posixct)+
  ...
  Plot2<-(ggplot(data=subset(All.Runs,variable=="S"), aes(x=date.time1,y=value,color=L2))+
            geom_point(alpha=0.3)+
            ggtitle(paste0("Specific Stage for USGS Gage #", Current_Site))+
            theme(plot.title=element_text(hjust=0.5))+
            guides(colour = guide_legend(override.aes = list(alpha = 1)))+
            scale_color_discrete(name="Discharge\nQuantile",breaks=rev(Run.names),labels=rev(paste(colnames(quantiles),":",round(quantiles,0)," cfs")))+
            scale_x_datetime(date_labels = "%b\n%Y",limits = UVStage_Range.posixct, date_minor_breaks = "1 year")+
            ylab("Stage (ft)"))+
  ...
  print("Plotting Target Quantile with hydrograph")
      Target_D <-round(quantiles[Target_Quantile],2)
          ...
          Plot3<-(ggplot(data=subset(All.Runs,variable=="S"&L2==Run.names[Target_Quantile]), aes(x=date.time1,y=value))+
  ...
  #     
  #     Plot4<-(ggplot(data=subset(All.Runs.Target), aes(x=date.time1,y=SS))+
  #               ggtitle(paste0("Stage at USGS Gage #", Current_Site,"\n", colnames(quantiles)[Target_Quantile]," Discharge Quantile: ",Target_D, " cfs"))+
  #               theme(plot.title=element_text(hjust=0.5))+
