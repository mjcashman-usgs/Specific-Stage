...
theme_ss_facet <- function (size) { 
    theme_bw(base_size=size, base_family="sans") +
        theme(strip.background = element_rect(fill="lightgrey", color = "black", size=0.2),
...
p
ggsave(plot = p, filename = "./Manuscript_Output/DV_Discharge.png", height = height, width =  width)
...
#ggplotly(p)
ggsave(plot = p, filename = "./Manuscript_Output/UV_Discharge.png", height = height, width = width)
```
...
p<-ggplot(data=uv_data, aes(x=dateTime, y=elev_GH))+
    ...
    scale_x_datetime(date_labels = "%Y",date_breaks = "1 year",limits= as.POSIXct(strptime(c(start,end), format = "%Y-%m-%d")))
#ggplotly(p)
ggsave(plot = p, filename = "./Manuscript_Output/UV_Stage.png", height = height, width = width)
```
...
fwrite(SS_all_results,paste0(writeDir,"/SQ_all_results.csv"))
```
...
p<-ggplot(All_SQ,aes(x =dateTime , y = Flow_Inst, color = as.factor(Quantflow))) +
...
p <- ggplot(data=subset(SS_all_results), aes(x=dateTime,y=GH_Inst,color=as.factor(i)))+
 ...
 # ggtitle(paste("USGS ", Site, sitedata$station_nm), subtitle = "Specific Stage for all Flow Quantiles")+
  ...
  geom_smooth(data = SS_all_results %>% filter(Quantile==0.3), span=0.3, method='loess', se=FALSE) +
  geom_smooth(data = SS_all_results %>% filter(Quantile==0.5), span=0.3, method='loess', se=FALSE) +
  geom_smooth(data = SS_all_results %>% filter(Quantile==0.7), span=0.3, method='loess', se=FALSE) +
  ...
  geom_smooth(data = SS_all_results %>% filter(Quantile==0.9975), span=0.4, method='loess', se=FALSE) +
  geom_smooth(data = SS_all_results %>% filter(Quantile==0.999), span=0.6, method='loess', se=FALSE) +
  geom_smooth(data = SS_all_results %>% filter(Quantile==0.9999), span=0.7, method='loess', se=FALSE) +
  geom_smooth(data = SS_all_results %>% filter(Quantile==0.9999999), span=0.7, method='loess', se=FALSE) +
...
```{r Inverse Plot - What discharge per stage?}

ggplot(data=subset(SS_all_results,Quantile==0.9999999), aes(x=dateTime, y=Flow_Inst, color=as.factor(site_no)))+
 # geom_hline(data = All_Flood, aes(yintercept = stage), color=All_Flood$color, linetype=5,size=1.05) +
  geom_point(alpha=1)+
    scale_x_datetime(date_labels = "%Y",date_breaks = "1 year",limits= as.POSIXct(strptime(c(start,end), format = "%Y-%m-%d"))) +
    facet_wrap(~full_name, ncol=1, scales="free_y")
...
p <- ggplot(data, aes(x=discharge_va, y=-chan_depth, color=as.factor(Year),group=as.factor(Year))) +
  geom_point(alpha=0.5)+
  scale_color_viridis_d()  +
  # ggtitle(paste0("Estimated Froude number between ",Q_window_low,"-",Q_window_high," cfs"))+
  theme_ss_facet(18)+
...
data[site_no == "01589000", y_min := 185]
data[site_no == "01589000", y_max := 190]
data[site_no == "01589025", y_min := 73]
data[site_no == "01589025", y_max := 78]
data[site_no == "01589035", y_min := 11]
data[site_no == "01589035", y_max := 16]

p <- ggplot(data, aes(x=discharge_va, y=gage_height_va-chan_depth+alt_va, color=as.factor(Year),group=as.factor(Year))) +
...
#  scale_y_log10()+
  scale_x_log10()+
  ylab("Corrected Bed Elevation (ft NAVD88)")+
  xlab("Field Measured Discharge (cfs)")+
...
p<-ggplot(data,aes(x=measurement_dateTime, y=gage_height_va-chan_depth+alt_va, color=discharge_va)) +
  geom_point()+
  scale_color_viridis_c()  +
    ggtitle(paste0("Corrected bed elevation at ",Q_window_low,"-",Q_window_high," cfs"))+
  theme_ss_facet(18)+
  geom_smooth(alpha=0.3, color="grey", span=0.25, se=FALSE)+
  geom_vline(xintercept=as.numeric(as.POSIXct(dam_start)), linetype=4)+
  geom_vline(xintercept=as.numeric(as.POSIXct(Bloede_start)), linetype=4)+
  ...
  ylab("Corrected bed elevation (ft NAVD88)")
p
ggsave(plot = p, filename = "./Manuscript_Output/Bed_ts_50500.png", height = height, width = width)
#ggplotly(p)
p <- ggplot(data, aes(x=discharge_va, y=gage_height_va-chan_depth+alt_va, color=as.factor(Year),group=as.factor(Year))) +
  geom_point(alpha=0.8)+
  scale_color_viridis_d()  +
...
p <- ggplot(data, aes(x=discharge_va, y=gage_height_va-chan_depth, color=as.factor(Year),group=as.factor(Year))) +
  geom_point(alpha=0.8)+
  scale_color_viridis_d()  +
  # ggtitle(paste0("Estimated Froude number between ",Q_window_low,"-",Q_window_high," cfs"))+
  theme_ss_facet(18)+
...
#  scale_y_log10()+
  scale_x_log10()+
  ylab("Estimated Bed Residual (ft)")+
  xlab("Field Measured Discharge (cfs)")+
  facet_wrap(~full_name, ncol=1)+
...
```{r}
data <- dv_data %>%
  select(Date, site_no,Flow,Turb,`80155`,full_name) %>%
    na.omit() %>% group_by(site_no) %>%  
    filter(Flow!=-999999) %>%
    filter(Turb!=-999999) %>%
    mutate(cumQ=cumsum(Flow),cumSSC=cumsum(`80155`)) %>% ungroup() 
p <- ggplot(data, aes(x=Date, y=cumT, group=site_no, color=full_name))+
  geom_path(size=2)+
  theme_ss_facet(18)+
